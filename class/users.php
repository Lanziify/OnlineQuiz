...
class users
{
	...
	{
		$query = $this->conn->query("SELECT username, pass FROM admin WHERE username='$username' AND pass='$pass'");
		$query->fetch_array(MYSQLI_ASSOC);
		if ($query->num_rows > 0) {
	...
	{
		$this->conn->query("insert into category values ('','$category')");
		return true;

	}
	public function delete_category($id)
	{
		$this->conn->query("DELETE FROM category WHERE id = '$id'");
		return true;
	}
	public function update_category($id, $cat)
	{
		$this->conn->query("UPDATE category SET category='$cat' WHERE id='$id'");
		return true;
	}
